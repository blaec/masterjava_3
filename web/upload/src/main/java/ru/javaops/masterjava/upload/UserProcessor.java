...
public class UserProcessor {
     ...
     */
    public List<FailedEmails> process(final StaxStreamProcessor processor, Map<String, City> cities, Set<String> groups, int chunkSize) throws XMLStreamException, JAXBException {
        ...
        while (processor.doUntil(XMLEvent.START_ELEMENT, "User")) {
            ...
            } else if (groupRefs.size() == 1 && groupRefs.get(0).isEmpty()) {
                failed.add(new FailedEmails(xmlUser.getEmail(), "This user not belongs to any group"));
            } else if (groupRefs.stream().anyMatch(gr -> !groups.contains(gr))) {
                failed.add(new FailedEmails(xmlUser.getEmail(), "One of user groups '" + groupRefs + "' is not present in DB"));
            } else {
                final User user = new User(id++, xmlUser.getValue(), xmlUser.getEmail(), UserFlag.valueOf(xmlUser.getFlag().value()), cityRef);
                chunk.add(user);
                userGroups.addAll(groupRefs.stream()
                        .map(g -> new UserGroup(user.getId(), groupDao.getGroupId(g)))
        ...
        userGroupDao.insertBatch(userGroups.stream()
                .filter(ug -> userDao.getUserId(ug.getUserId()) > 0)
                .collect(Collectors.toList()));
